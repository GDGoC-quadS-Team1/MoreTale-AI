...
def _load_illustration_manifest(
    ...
    try:
        ...
        return {}, None, None, to_static_outputs_url(manifest_path)
    ...
    return entry_map, cover_entry, manifest_summary, to_static_outputs_url(manifest_path)


def _load_vocabulary_manifest(
    run_dir: Path,
...
def _find_manifest_asset_url(run_dir: Path, entry: dict[str, Any]) -> str | None:
...
def _find_illustration_url_from_entry(run_dir: Path, entry: dict[str, Any]) -> str | None:
    return _find_manifest_asset_url(run_dir=run_dir, entry=entry)
...
def _first_existing_cover_url(run_dir: Path) -> str | None:
    illustrations_dir = run_dir / "illustrations"
    if not illustrations_dir.is_dir():
        return None
    ...
    *,
    ...
    for index, raw_entry in enumerate(raw_entries, start=1):
        ...
        if primary_manifest_entry is not None:
            ...
            primary_url = _find_manifest_asset_url(run_dir=run_dir, entry=primary_manifest_entry)
            if primary_url is None and has_primary_audio:
                primary_url = f"{static_prefix}/{primary_rel.as_posix()}"
        ...
        if secondary_manifest_entry is not None:
            ...
            secondary_url = _find_manifest_asset_url(run_dir=run_dir, entry=secondary_manifest_entry)
            if secondary_url is None and has_secondary_audio:
                secondary_url = f"{static_prefix}/{secondary_rel.as_posix()}"
...
def build_story_result_payload(
    ...
    vocabulary_entry_map, vocabulary_manifest_exists = _load_vocabulary_manifest(run_dir)
    ...
    if not cover_enabled:
        ...
        if cover_manifest_entry is not None:
            ...
            cover_url = _find_illustration_url_from_entry(run_dir, cover_manifest_entry)
        else:
            cover_url = _first_existing_cover_url(run_dir)
            if cover_url:
    ...
    has_partial_failures = bool(
        ...
        or (
            cover_enabled
            and cover_summary["status"] in {"failed", "missing"}
        )
    )
